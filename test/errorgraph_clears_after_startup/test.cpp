...
bool Tester::test(void) {
  ...
  while (rclcpp::ok() && elapsed < timeout_s) {
    ...
    if (!has_waiting_for_node) {
      consecutive_clean++;
      RCLCPP_INFO(node_->get_logger(), "Clean message %d/%d (errors: %zu)", consecutive_clean,
                   required_consecutive_clean, element.errors.size());
    } else {
      consecutive_clean = 0;
    }
  ...
  if (consecutive_clean < required_consecutive_clean) {
    RCLCPP_ERROR(node_->get_logger(),
                 ...
                 timeout_s, saw_any_waiting_error ? "true" : "false", consecutive_clean,
  ...
  if (!saw_any_waiting_error) {
    RCLCPP_WARN(node_->get_logger(),
                "WARNING: errors cleared but no waiting_for_node errors were observed "
                "during startup (startup window may have been missed)");
  }

  RCLCPP_INFO(node_->get_logger(),
              "SUCCESS: errorgraph errors cleared after startup (%d consecutive clean messages).",
              consecutive_clean);
  return true;
}
